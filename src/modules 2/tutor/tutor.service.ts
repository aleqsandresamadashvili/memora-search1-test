...
export class TutorService {
  async step1(userId: string, data: { firstName: string; lastName: string; phone: string; dob: string }) {
    ...
    return { user, smsCode: code } as const;
  }

  async verifyPhone(userId: string, code: string) {
    const user = await prisma.user.findUnique({ where: { id: userId } });
    if (!user || !user.phoneVerificationCode || !user.phoneVerificationExpiresAt) return { ok: false } as const;
    if (user.phoneVerificationCode !== code || user.phoneVerificationExpiresAt < new Date()) return { ok: false } as const;
  ...
  async step2(userId: string, data: {
    subjects: { subjectId: string; hourlyPrice: number }[];
    ...
    if (data.subjects.length === 0 || data.subjects.length > 5) throw new Error('Select 1 to 5 subjects');
